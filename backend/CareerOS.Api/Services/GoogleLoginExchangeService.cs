...
public class GoogleLoginExchangeService
{
    ...
    private static readonly ConcurrentDictionary<string, OAuthStatePayload> _oauthStates = new();
    ...
    public string CreateOAuthState()
    {
        ...
        _oauthStates[state] = new OAuthStatePayload();
        CleanExpiredStates();
        return state;
    }
    ...
    public bool TryConsumeOAuthState(string? state)
    {
        ...
        if (_oauthStates.TryRemove(state, out var payload))
        {
            if (DateTimeOffset.UtcNow - payload.CreatedAt <= TimeSpan.FromSeconds(120))
            {
                return true;
            }
        }
    ...
    {
        var now = DateTimeOffset.UtcNow;
        foreach (var kvp in _oauthStates)
        {
            if (now - kvp.Value.CreatedAt > TimeSpan.FromSeconds(120))
            {
                _oauthStates.TryRemove(kvp.Key, out _);
            }
        }
    }
}
