...
MOUNTS_JSON=$(curl -s -H "X-Vault-Token: ${BAO_TOKEN}" "${BAO_ADDR}/v1/sys/mounts")
...
AUTH_JSON=$(curl -s -H "X-Vault-Token: ${BAO_TOKEN}" "${BAO_ADDR}/v1/sys/auth")
...
ROLE_ID_RESP=$(curl -s -H "X-Vault-Token: ${BAO_TOKEN}" "${BAO_ADDR}/v1/auth/approle/role/careeros/role-id")
ROLE_ID=$(echo "${ROLE_ID_RESP}" | grep -o '"role_id":"[^"]*' | cut -d'"' -f4)

SECRET_ID_RESP=$(curl -s -H "X-Vault-Token: ${BAO_TOKEN}" -X POST "${BAO_ADDR}/v1/auth/approle/role/careeros/secret-id")
SECRET_ID=$(echo "${SECRET_ID_RESP}" | grep -o '"secret_id":"[^"]*' | cut -d'"' -f4)
